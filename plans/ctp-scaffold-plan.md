...
- **Overnight run — fully automated:** The user has pre-approved all phases. Do not stop for plan approval, phase commit confirmation, or completion review. Commit after each phase automatically using the generated commit message and proceed immediately to the next phase. Run all 8 phases to completion without pausing.
